...
IMAGE_REPOSITORY    := $(REGISTRY)/control-plane-coordinator
IMAGE_TAG           := $(VERSION)
BUILD_DIR           := build
BIN_DIR             := bin
...
.PHONY: docker-image
docker-image: 
	@if [[ ! -f $(BIN_DIR)/linux-amd64/control-plane-coordinator ]]; then echo "No binary found. Please run 'make build'"; false; fi
